...
Our goal here is to setup each job can with the connection it needs, so the job must know which client it belongs to.
...
We need to override the method to register the `DatabaseConnector` in order to use our own, which overrides
